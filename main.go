...
/*
VGA 80x25 characters 9 pixels wide by 14 pixels high, 720x350 pixels
from character 192 to 223 the 9th column is 8th column repetition

*/

var screenWidth uint = 9
var screen = [9 * 14]rune{}
...
func drawPix(x, y uint, c rune) {
	pos := y*screenWidth + x
	screen[pos] = c
}

func drawChar(index uint, x, y uint) {
	var a uint
	var b uint
	for a = 0; a < 9; a++ {
		for b = 0; b < 14; b++ {
...
func main() {
	...
	for i = 0; i <= 255; i++ {
		...
		for a = 0; a < 14; a++ {
			...
			var b uint
			for b = 0; b < 9; b++ {
				fmt.Print(string(screen[a*screenWidth+b]))
			}
			fmt.Println()
		}
	}
